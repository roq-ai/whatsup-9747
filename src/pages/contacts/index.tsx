...
type ColumnType = ColumnDef<ContactInterface, unknown>;
...
export function ContactListPage(props: ContactListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getContacts({
        relations: ['business', 'message.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ContactInterface>>(
    () => `/contacts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('message', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Message',
          accessorKey: 'message',
          cell: ({ row: { original: record } }: any) => record?._count?.message,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/contacts/view/${record.id}`} passHref legacyBehavior>
            <Button
